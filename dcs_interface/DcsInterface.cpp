...
void DcsInterface::update_dcs_state()
{
    ...
    if (std::getline(recv_msg, token, header_delimiter)) {
        ...
        while (utilities::pop_key_and_value(recv_msg, ':', '=', key_and_value)) {
...
std::optional<utilities::Decimal> DcsInterface::get_decimal_of_dcs_id(const int dcs_id)
{
    if (current_game_state_.count(dcs_id) > 0) {
        if (utilities::is_number(current_game_state_[dcs_id])) {
            return current_game_state_[dcs_id];
        }
    }
    return std::nullopt;
...
void DcsInterface::send_dcs_reset_command() { dcs_socket_.send("R"); }
