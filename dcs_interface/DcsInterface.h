...
class DcsInterface
{
  public:
     ...
     *
     * @return Optional value of DCS ID is returned if a value has been logged.
     */
    std::optional<std::string> get_value_of_dcs_id(const int dcs_id);

    /**
     * @brief Get the value as Decimal of dcs id object from current game state.
     *
     * @return Optional value of DCS ID is returned if a value has been logged that is a Decimal.
     */
    std::optional<utilities::Decimal> get_decimal_of_dcs_id(const int dcs_id);
  ...
  private:
    ...
    utilities::UdpSocket dcs_socket_;           // UDP Socket connection for communicating with DCS lua export scripts.
