...
#include "../dcs_interface/DcsInterface.h"

namespace dcs_interface::test
{
...
TEST(DcsInterfaceTest, dcs_reset_command_on_construction)
{
    ...
    utilities::UdpSocket mock_dcs(
        connection_settings.ip_address, connection_settings.tx_port, connection_settings.rx_port);
...
class DcsInterfaceTestFixture : public ::testing::Test
{
  public:
    ...
    DcsConnectionSettings connection_settings = {"1908", "1909", "127.0.0.1"};
    utilities::UdpSocket mock_dcs; // A socket that will mock Send/Receive messages from DCS.
...
TEST_F(DcsInterfaceTestFixture, get_decimal_of_dcs_id_with_valid_value)
{
    ...
    EXPECT_TRUE(maybe_value);
    EXPECT_EQ(maybe_value.value(), utilities::Decimal("99.99"));
}
